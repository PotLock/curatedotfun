...
const SubmissionList = ({ items, statusFilter, botId, feedId }: SubmissionListProps) => {
    ...
    return (
      <div className="flex flex-col justify-center items-center p-8 space-y-2">
        ...
        <p className="text-gray-400 text-sm">
          comment with "!submit @{botId} #{feedId || 'feedId'}" to start curating
        </p>
      </div>
    );
  }
  ...
  return (
    <>
      {filteredItems
        ...
        .map((item) => <FeedItem key={item.tweetId} submission={item} statusFilter={statusFilter || "all"} />)}
    </>
  );
};
