...
type GetUserActivitiesOutput = z.infer<typeof getUserActivitiesDefinition.output>;
type GetMyCuratedFeedsOutput = z.infer<typeof getMyCuratedFeedsDefinition.output>;
type GetMyApprovedFeedsOutput = z.infer<typeof getMyApprovedFeedsDefinition.output>;
...
export function useGlobalActivityStats(
  ...
  const trpc = useTRPC();
  return useQuery(trpc.activity.getGlobalStats.queryOptions(undefined, {
    ...options,
...
export function useLeaderboard(
  ...
  const trpc = useTRPC();
  return useQuery(trpc.activity.getLeaderboard.queryOptions(input, {
    ...options,
    select: options?.select ?? ((data: GetLeaderboardOutput) => data.leaderboard),
  }));
}

export function useMyActivities(
  input: GetMyActivitiesInput,
  options?: Omit<UseQueryOptions<GetMyActivitiesOutput>, 'queryKey' | 'queryFn'>
) {
  const trpc = useTRPC();
  const { web3auth } = useWeb3Auth();
  return useQuery(trpc.activity.getMyActivities.queryOptions(input, {
    ...options,
    enabled: !!web3auth && (options?.enabled !== undefined ? options.enabled : true),
  }));
}

export function useUserActivities(
  input: GetUserActivitiesInput,
  options?: Omit<UseQueryOptions<GetUserActivitiesOutput>, 'queryKey' | 'queryFn'>
) {
  const trpc = useTRPC();
...
export function useMyCuratedFeeds(
  options?: Omit<UseQueryOptions<GetMyCuratedFeedsOutput>, 'queryKey' | 'queryFn'>
) {
  const trpc = useTRPC();
  const { web3auth } = useWeb3Auth();
  return useQuery(trpc.activity.getMyCuratedFeeds.queryOptions(undefined, {
     ...options,
    enabled: !!web3auth && (options?.enabled !== undefined ? options.enabled : true),
  }));
}
...
export function useMyApprovedFeeds(
  options?: Omit<UseQueryOptions<GetMyApprovedFeedsOutput>, 'queryKey' | 'queryFn'>
) {
  const trpc = useTRPC();
  const { web3auth } = useWeb3Auth();
  return useQuery(trpc.activity.getMyApprovedFeeds.queryOptions(undefined, {
    ...options,
    enabled: !!web3auth && (options?.enabled !== undefined ? options.enabled : true),
  }));
}
...
export function useMyFeedRank(
  ...
  const trpc = useTRPC();
  const { web3auth } = useWeb3Auth();
  return useQuery(trpc.activity.getMyFeedRank.queryOptions(input, {
    ...options,
    enabled: !!web3auth && (options?.enabled !== undefined ? options.enabled : true),
  }));
}
