import { SourceItem, SourcePlugin, LastProcessedState, PlatformState, SourcePluginSearchOptions } from "@curatedotfun/types";
import { FeedConfig, SourceConfig, SourceSearchConfig } from "../types/config.zod";
...
export class SourceService {
  ...
  private async fetchFromSearchConfig(
    ...
    const { searchId, type, query, pageSize, platformArgs, ...rest } = searchConfig;
    ...
    try {
      ...
      if (results.nextLastProcessedState) {
        ...
        logger.info(`Plugin ${sourcePluginName} (searchId: ${searchId}) returned no nextLastProcessedState.`);
  ...
  public async fetchFromSource(
    ...
    const { plugin: pluginName, config: pluginInstanceConfig, search: searchConfigs } = sourceConfig;
    ...
    for (const searchConfig of searchConfigs) {
      try {
        ...
        logger.error(`Skipping searchId ${searchConfig.searchId} for plugin ${pluginName} due to error.`);
      }
    }
    return allItems;
  }
   ...
   */
  public async fetchAllSourcesForFeed(feedConfig: FeedConfig): Promise<SourceItem[]> {
    ...
    for (const sourceConfig of feedConfig.sources) {
      try {
        ...
        logger.error(`Skipping source plugin ${sourceConfig.plugin} for feed ${feedConfig.id} due to error.`);
      }
    }

    logger.info(`Total items fetched for feed '${feedConfig.id}': ${allItemsFromFeed.length}`);
  ...
  async shutdown(): Promise<void> {
    ...
    logger.info("SourceService shutdown initiated. Relies on PluginService for plugin cleanup.");
  }
}
