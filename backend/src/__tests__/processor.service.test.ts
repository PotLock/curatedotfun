...
import { ProcessorError, TransformError, TransformStage } from "../types/errors";
import { DistributorConfig, TransformConfig } from "../types/config";

class MockTransformationService {
  async applyTransforms(content: any, transforms: TransformConfig[], stage: TransformStage) {
...
describe("ProcessorService", () => {
  ...
  describe("process", () => {
    ...
    it("should handle distributor-specific transforms", async () => {
      ...
      mockTransformationService.applyTransforms = async (content: any, _transforms: TransformConfig[], stage: TransformStage) => {
    ...
    it("should continue with other distributors if one fails", async () => {
      ...
      let firstCall = true;
      mockDistributionService.distributeContent = async (distributor: DistributorConfig, content: any) => {
        if (firstCall) {
          firstCall = false;
      ...
      expect(mockDistributionService.distributedItems).toHaveLength(1);
      expect(mockDistributionService.distributedItems[0].distributor.plugin).toBe("success");
    });
    ...
    it("should throw if all distributors fail", async () => {
      ...
      await expect(processorService.process(mockContent, config)).rejects.toThrow(ProcessorError);
    });
    ...
    it("should continue with original content on transform error", async () => {
      ...
      expect(mockDistributionService.distributedItems[0].content).toEqual(mockContent);
    });
    ...
    it("should throw error if no distributors configured", async () => {
      ...
      await expect(processorService.process(mockContent, config)).rejects.toThrow(
  ...
  describe("processBatch", () => {
    ...
    it("should continue with untransformed batch on batch transform error", async () => {
      ...
      mockTransformationService.applyTransforms = async (_content: any, _transforms: TransformConfig[], stage: TransformStage) => {
        if (stage === "batch") {
      ...
      expect(mockDistributionService.distributedItems[0].content).toEqual(mockItems);
    });
    ...
    it("should handle individual item processing failures", async () => {
      ...
      const distributedContent = mockDistributionService.distributedItems[0].content;
      expect(distributedContent).toHaveLength(2);
      expect(distributedContent[0]).toEqual(mockItems[0]); // Failed, original
      expect(distributedContent[1]).toEqual({ ...mockItems[1], transformed: true }); // Succeeded
    });
  });
});
