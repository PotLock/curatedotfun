...
export const QueryOptionsSchema = z.object({
  limit: z.string().optional().transform(val => val ? parseInt(val, 10) : undefined).pipe(z.number().int().min(1).optional()),
  offset: z.string().optional().transform(val => val ? parseInt(val, 10) : undefined).pipe(z.number().int().min(0).optional()),
  sortBy: z.string().optional(),
  sortOrder: z.enum(["asc", "desc"]).optional(),
});
export type BaseStringQueryOptions = z.infer<typeof QueryOptionsSchema>;

export const PaginatedDataSchema = <T extends z.ZodTypeAny>(itemSchema: T) => z.object({
