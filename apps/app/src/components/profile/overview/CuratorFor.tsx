...
export function CuratorFor() {
  const { data: userCuratedFeeds, isLoading, isError } = useMyCuratedFeeds();
  ...
  return (
    <div className="border rounded-lg px-2 sm:px-3 pb-3 border-neutral-300 gap-2 sm:gap-3 flex flex-col">
      ...
      <div className="grid gap-1">
        ...
        {!isLoading && !isError && userCuratedFeeds?.length === 0 && (
          <div className="flex items-center justify-center h-24">
        ...
        {!isLoading &&
          ...
          userCuratedFeeds.map((feed) => <Item key={feed.id} data={feed} />)}
      </div>
    </div>
  );
}
