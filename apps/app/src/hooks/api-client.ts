...
>(
  ...
  const queryFunction = () => apiClient.makeRequest<TQueryFnData>(
    'GET',
    path,
    { currentAccountId, isSignedIn }
  );

  const finalQueryOptions: UseQueryOptions<TQueryFnData, TError, TData, TQueryKey> = {
    queryKey,
...
>(
  ...
  const mutationFunction = async (variables: TVariables) => { // 'variables' is the data passed to mutate(), used as requestData
  ...
  const finalMutationOptions: UseMutationOptions<TData, TError, TVariables, TContext> = {
...
>(
  ...
  const queryFunction = (context: QueryFunctionContext<TQueryKey, TPageParam>) => {
    const path = pathFn(context.pageParam as TPageParam); 
    return apiClient.makeRequest<TQueryFnData>(
      'GET',
      path,
      { currentAccountId, isSignedIn }
    );
  ...
  const finalQueryOptions: UseInfiniteQueryOptions<TQueryFnData, TError, TData, TQueryKey, TPageParam> = {
    queryKey,
    queryFn: queryFunction,
    ...options,
  };

  return useInfiniteQuery<TQueryFnData, TError, TData, TQueryKey, TPageParam>(finalQueryOptions);
}
