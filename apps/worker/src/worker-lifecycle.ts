...
export function initializeWorkers(): Worker<any>[] {
  ...
  for (const config of workerConfigurations) {
    const worker = createWorkerInstance(config.name, config.processor, config.opts);

    // --- Common BullMQ Worker Event Listeners ---
    // These listeners apply to all workers created via this initializer
    worker.on('completed', (job) => {
      console.log(`[Event:Completed] Job ${job.id} on queue '${job.name}' finished.`);
    });

    worker.on('failed', (job, err) => {
      console.error(`[Event:Failed] Job ${job?.id} on queue '${job?.name}' failed with error:`, err);
    });

    worker.on('error', (err) => {
      console.error(`[Event:Error] Worker for queue '${worker.name}' experienced an unhandled error:`, err);
    });

    worker.on('active', (job) => {
      console.log(`[Event:Active] Job ${job.id} on queue '${job.name}' is now active.`);
    });

    worker.on('stalled', (jobId) => {
      console.warn(`[Event:Stalled] Job ${jobId} on queue '${worker.name}' stalled.`);
    });
...
export async function gracefulShutdown(workers: Worker<any>[], queues: Queue<any>[]): Promise<void> {
